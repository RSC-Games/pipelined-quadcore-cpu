...
// instruction opcode. Minor <potential> speed optimization on cacheless architectures.
DecodedInstruction* (*instr_dispatch[256])(uint32_t) = {
...
DecodedInstruction* _decode_instr_3reg(uint32_t instr) {
    uint32_t* operands = new uint32_t[3];
    operands[0] = instr >> 20 & 0xF;
    operands[1] = instr >> 16 & 0xF;
    operands[2] = instr >> 12 & 0xF;
...
DecodedInstruction* _decode_instr_4reg(uint32_t instr) {
    uint32_t* operands = new uint32_t[4];
    operands[0] = instr >> 20 & 0xF;
    operands[1] = instr >> 16 & 0xF;
    operands[2] = instr >> 12 & 0xF;
    operands[3] = instr >> 8 & 0xF;
...
// Decode a singular 20 bit immediate.
DecodedInstruction* _decode_instr_20bimm(uint32_t instr) {
    uint32_t* operands = new uint32_t[1];
    operands[0] = instr & 0xFFFFF;
...
// Decode a register and a 20 bit immediate.
DecodedInstruction* _decode_instr_reg_imm(uint32_t instr) {
    uint32_t* operands = new uint32_t[2];
    operands[0] = instr >> 20 & 0xF;
    operands[1] = instr & 0xFFFFF;
...
DecodedInstruction* decode_instr(uint32_t instr) {
    ...
    isa::DecodedInstruction* decoded = decoder(instr);
    decoded->instruction = instr_opcode;
    return decoded;
}
}